	...
	*/
	...
	public function add_interview($addarr,$user){
		...
		if($mailconfig['set_invite'] == 1){
			...
			D('Mailconfig')->send_mail(array('sendto_email'=>$useremail['email'],'send_type'=>'set_invite'),array('companyname'=>$job_info['companyname']));
		}
		//sms
		if(C('qscms_sms_open')==1 && $sms['set_invite']== 1 && $addarr['sms_notice'] == 1){
			...
			if($send_sms==true){
				...
				$r = D('Sms')->sendSms('notice',array('mobile'=>$usermobile['mobile'],'tpl'=>'set_invite','data'=>array('companyname'=>$job_info['companyname'])));
				if($r===true){
