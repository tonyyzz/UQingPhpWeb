		...
		if (fieldRegex($sqlin,'in'))
		{
			...
			if ($audit=="1" && $mailconfig['set_jobsallow']=="1")//审核通过
			{
				...
				foreach ($result as $key => $value) {
					...
					if($useremail['email']){
						$replace_arr['jobsname'] = $value['jobs_name'];
						D('Mailconfig')->send_mail(array('sendto_email'=>$useremail['email'],'send_type'=>'set_jobsallow'),$replace_arr);
					}
				}
			}
			if ($audit=="3" && $mailconfig['set_jobsnotallow']=="1")//审核未通过
			{
				...
				foreach ($result as $key => $value) {
					...
					if($useremail['email']){
						$replace_arr['jobsname'] = $value['jobs_name'];
						D('Mailconfig')->send_mail(array('sendto_email'=>$useremail['email'],'send_type'=>'set_jobsnotallow'),$replace_arr);
					}
				}
			}
			//sms
