...
class ResumeModel extends RelationModel{
	...
	*/
	public function check_resume($uid,$pid)
	{
		...
		/* 分词 start */
		$setsqlarr['key_full'] = $setsqlarr['key_precise'] = $resume['intention_jobs'].$resume['education_cn'];
	...
	public function admin_edit_resume_photo_audit($id,$audit,$audit_man='')
	{
		!is_array($id) && $id=array($id);
		$sqlin=implode(",",$id);
		if (fieldRegex($sqlin,'in'))
		{
			...
			$resume_list = $this->field('id,uid,photo_img,photo_display')->where(array('id'=>array('in',$sqlin)))->select();
			foreach ($resume_list as $key => $val) {
