...
function url_rewrite($alias=NULL,$get=NULL,$subsite_id=0,$rewrite=true){
    if(C('PLATFORM')=='mobile' && MODULE_NAME == 'Mobile'){
    ...
    if($page_list[$alias]['url']=='0' || $rewrite==false){//原始链接
        if(MODULE_NAME != C('DEFAULT_MODULE') || $page_list[$alias]['module'] != strtolower(MODULE_NAME)) $u = $page_list[$alias]['module'].'/';
        $u .= $page_list[$alias]['controller'].'/'.$page_list[$alias]['action'];
        $url = U($u,$get,true,false,true);
    }else{
