...
class CompanyServiceController extends CompanyController{
    ...
    public function service_resume(){
    	if(IS_POST){
			...
			if(!$return_order_info){
				...
				write_members_log(C('visitor'),9002,$return_order_info['service_name'],$log_payment);
    ...
    public function service_stick(){
    	if(IS_POST){
			...
			if(!$return_order_info){
	            ...
	            {
	            	/* 会员日志 */
					write_members_log(C('visitor'),9002,$service_info['name'],C('qscms_points_byname').'兑换');
    ...
    public function service_refresh(){
    	if(IS_POST){
			...
			$params_array['endtime'] = $params_array['starttime']+3600*6*4;
			$params_array['jobs_id'] = $jobs_id;
			$params['params'] = serialize($params_array);
			$return_order_info = $this->_order_insert($params);
			if(!$return_order_info){
    ...
    public function gold_add(){
    	if(IS_POST){
        	...
        	$this->assign('sys_max',C('qscms_com_buy_points_max'));
        	$this->assign('payment_rate',C('qscms_payment_rate'));
    		$this->assign('my_points',$this->my_points);
    ...
    public function setmeal_add(){
    	if(IS_POST){
			...
			if(!$return_order_info){
				...
				if($return_order_info['pay_points']>0){
					...
					if($p_rst){
		                ...
		                $notes=date('Y-m-d H:i',$this->timestamp)."通过：".C('qscms_points_byname')."兑换成功开通{$return_order_info['service_name']}";
						/* 会员日志 */
						write_members_log(C('visitor'),9002,$return_order_info['service_name'],C('qscms_points_byname').'兑换');
    		...
    		$setmeal_list = D('Setmeal')->where(array('display'=>1,'apply'=>1))->select();
    		foreach ($setmeal_list as $key => $value) {
    			...
    			$setmeal_list[$key]['service_points'] = round($value['expense']*C('qscms_payment_rate'));
    		}
    ...
    protected function _get_service_list($cat){
		...
		foreach ($increment_arr as $key => $value) {
			...
			//当前会员需要付的价格
			$increment_arr[$key]['service_price'] = $free_discount>0?round($value['price']*$free_discount/10,1):$value['price'];
			//VIP会员价格,取出折扣最大的套餐折扣
