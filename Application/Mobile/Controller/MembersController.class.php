...
class MembersController extends MobileController{
    ...
    public function register(){
        ...
        if(IS_POST && IS_AJAX){
            ...
            if($data['reg_type'] == 2 && C('qscms_check_reg_email')){//是否需要邮箱激活
                ...
                $url = C('qscms_site_domain').U('home/members/activate',array('key'=>$str),true,false,true);
    ...
    public function activate(){
        ...
        !$user && $this->_404('帐号不存在！',U('members/register'));
        $points_rule = D('Task')->get_task_cache(2,1);
        ...
        if($user['status'] == 0){
            ...
            $passport = $this->_user_server();
            if(false === $uid = $passport->edit($user['uid'],$d)) $this->_404('帐号激活失败，请重新操作！',U('members/register'));
            $user['reg_type'] = 2;
