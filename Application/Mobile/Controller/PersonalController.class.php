...
class PersonalController extends MobileController{
    ...
    protected function _global_variable() {
        ...
        // 短信必须验证
        if (C('qscms_sms_open')==1 && C('qscms_login_per_audit_mobile')==1 && C('visitor.mobile_audit') == 0 && !in_array(ACTION_NAME, array('per_security_tel','resume_add'))){
    ...
    */
    public function resume_add(){
        ...
        if(C('qscms_resume_max') <= $resume_count = M('Resume')->where(array('uid'=>$uid))->count('id')){
            ...
            $this->_404('您最多可以创建'.C('qscms_resume_max').'份简历,已经超出了最大限制！');
        }else{
        ...
        if(IS_POST && IS_AJAX){
            ...
            $trims = array('telephone','fullname','email','intention_jobs_id','trade','district');
            foreach ($ints as $val) {
            ...
            foreach(array('major','marriage','householdaddress','residence') as $val){
    ...
    */
	public function resume_edit_description(){
        ...
        if(IS_AJAX){
            ...
            if(!$rst['state']) $this->ajaxReturn(0,$rst['error']);
            write_members_log(C('visitor'),2027,$resume['id']);
            $data['url'] = I('request.news',0,'intval') ? U('personal/resume_replenish',array('pid'=>$resume['id'],'news'=>1)) : U('personal/resume_edit',array('pid'=>$resume['id']));
    ...
    */
    public function resume_list(){
        ...
        $resume_list=D('Resume')->get_resume_list(array('where'=>$where,'order'=>'id asc','views'=>true,'countdown'=>true,'stick'=>true,'strong_tag'=>true));
        ...
        $this->_config_seo(array('title'=>'简历管理 - 个人会员中心 - '.C('qscms_site_name'),'header_title'=>'简历管理'));
        $this->display();
    }
    /**
     * [set_default 默认简历设置]
    ...
    */
    ...
    public function per_security(){
        $this->_config_seo(array('title'=>'帐号安全 - 个人会员中心 - '.C('qscms_site_name'),'header_title'=>'帐号安全'));
        $this->display();
    }
    ...
    public function user_info(){
        $uid = C('visitor.uid');
        $user_info=D('MembersInfo')->get_userprofile(array('uid'=>$uid));
        if(IS_AJAX){
            $ints = array('sex','birthday','education','major','experience','height','marriage');
            $trims = array('realname','residence','householdaddress');
            foreach ($ints as $val) {
                $setsqlarr[$val] = I('post.'.$val,0,'intval');
            }
            foreach ($trims as $val) {
            ...
            $this->_config_seo(array('title'=>'个人资料 - 个人会员中心 - '.C('qscms_site_name'),'header_title'=>'个人资料'));
            $this->display();
        }
    }
