	...
	<div class="tnav">
		...
		<div class="tl">
			<ul class="link_gray6 nowrap">
				<?php $tag_nav_class = new \Common\qscmstag\navTag(array('列表名'=>'nav','调用名称'=>'QS_top','显示数目'=>'10','cache'=>'0','type'=>'run',));$nav = $tag_nav_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"{companyname} - 职位列表 - {site_name}","keywords"=>"{companyname},职位列表","description"=>"{contents},职位列表","header_title"=>""),$nav);?>
...
<div class="comshow">
 ...
 <div class="cominfo">
        ...
        <div class="attention">
		    ...
		    <div class="fans">粉丝：<span><strong><?php echo ($info['fans']); ?></strong></span></div>
			<div class="clear"></div>
        </div>
...
<div class="comshowmain">
  ...
  <div class="l">
	<div class="comnav">
	 	 <a href="<?php echo url_rewrite('QS_companyshow',array('id'=>$info['id']));?>">公司主页</a>
		  <a href="<?php echo url_rewrite('QS_companyjobs',array('id'=>$info['id']));?>" class="select" >在招职位<span>(<?php echo ($info['jobs_count']); ?>)</span></a>
		  <div class="clear"></div>
	</div>
	
	<?php $tag_jobs_list_class = new \Common\qscmstag\jobs_listTag(array('列表名'=>'jobs','会员uid'=>$info['uid'],'显示数目'=>'-1','cache'=>'0','type'=>'run',));$jobs = $tag_jobs_list_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"{companyname} - 职位列表 - {site_name}","keywords"=>"{companyname},职位列表","description"=>"{contents},职位列表","header_title"=>""),$jobs);?>
	<div class="infobox link_blue">
      <?php if(!empty($jobs['list'])): if(is_array($jobs['list'])): $i = 0; $__LIST__ = $jobs['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="jobslist J_jobsList" data-jid="<?php echo ($vo['id']); ?>">
            <div class="jname"><a target="_blank" href="<?php echo ($vo['jobs_url']); ?>"><strong><?php echo ($vo['jobs_name']); ?></strong></a> <span>[<?php echo ($vo['amount']); ?>人]</span><span><?php echo ($vo['nature_cn']); ?></span><span><u><?php echo ($vo['refreshtime_cn']); ?></u></span></div>
            <div class="jtxt"><u><?php echo ($vo['wage_cn']); ?></u><span>|</span><?php echo ($vo['district_cn']); ?><span>|</span><?php echo ($vo['experience_cn']); ?><span>|</span><?php echo ($vo['education_cn']); ?></div>
            <div class="jobapp J_applyForJob" data-batch="false" data-url="<?php echo U('AjaxPersonal/resume_apply');?>">投递简历</div>
	...
	<?php if($similar['list']): ?><div class="same link_gray6">
      ...
      <?php if(is_array($similar['list'])): $i = 0; $__LIST__ = $similar['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="list link_gray6">
			...
			<div class="txt">
			    ...
			    <div class="count"><a target="_blank" href="<?php echo url_rewrite('QS_companyjobs',array('id'=>$vo['id']));?>"><span><?php echo ($vo['jobs_count']); ?></span></a>个招聘职位</div>
			</div>
			<div class="clear"></div>
	  </div><?php endforeach; endif; else: echo "" ;endif; ?>
	</div><?php endif; ?>
...
<div class="footer_min" id="footer">
	...
	<?php $tag_explain_list_class = new \Common\qscmstag\explain_listTag(array('列表名'=>'list','cache'=>'0','type'=>'run',));$list = $tag_explain_list_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"{companyname} - 职位列表 - {site_name}","keywords"=>"{companyname},职位列表","description"=>"{contents},职位列表","header_title"=>""),$list);?>
