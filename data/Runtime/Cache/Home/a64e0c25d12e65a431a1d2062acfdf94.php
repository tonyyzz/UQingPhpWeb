...
<head>
<?php $tag_news_category_class = new \Common\qscmstag\news_categoryTag(array('列表名'=>'info','资讯小类'=>$_GET['id'],'cache'=>'0','type'=>'run',));$info = $tag_news_category_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"资讯中心 -  {categoryname} -{site_name}","keywords"=>"","description"=>"","header_title"=>""),$info);?>
	...
	<div class="tnav">
		...
		<div class="tl">
			<ul class="link_gray6 nowrap">
				<?php $tag_nav_class = new \Common\qscmstag\navTag(array('列表名'=>'nav','调用名称'=>'QS_top','显示数目'=>'10','cache'=>'0','type'=>'run',));$nav = $tag_nav_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"资讯中心 -  {categoryname} -{site_name}","keywords"=>"","description"=>"","header_title"=>""),$nav);?>
...
<?php $tag_news_category_class = new \Common\qscmstag\news_categoryTag(array('列表名'=>'category','显示数目'=>'8','资讯大类'=>'1','cache'=>'0','type'=>'run',));$category = $tag_news_category_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"资讯中心 -  {categoryname} -{site_name}","keywords"=>"","description"=>"","header_title"=>""),$category);?>
<div class="newslip">
  <div class="l">
        <div class="newsnav">
		<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo ($vo['url']); ?>" <?php if($_GET['id']== $vo['id']): ?>class="select"<?php endif; ?>><?php echo ($vo['title_']); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
		<div class="clear"></div>
	</div>
	<?php $tag_news_list_class = new \Common\qscmstag\news_listTag(array('列表名'=>'list','关键字'=>$_GET['key'],'分页显示'=>'1','资讯小类'=>$_GET['id'],'填补字符'=>'…','摘要长度'=>'160','cache'=>'0','type'=>'run',));$list = $tag_news_list_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"资讯中心 -  {categoryname} -{site_name}","keywords"=>"","description"=>"","header_title"=>""),$list);?>
    <div class="newslist">
	  <div class="tit link_gray6">
		<?php if($_GET['key']!= ''): ?>与<strong><?php echo (urldecode(urldecode($_GET['key']))); ?></strong>有关的资讯
		<?php else: ?>
			<a href="<?php echo url_rewrite('QS_index');?>">网站首页</a>
			<?php if(!empty($list['breadcrumb'])): if(is_array($list['breadcrumb'])): $i = 0; $__LIST__ = $list['breadcrumb'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><span>></span><a href="<?php echo ($vo['url']); ?>"><?php echo ($vo['name']); ?></a><?php endforeach; endif; else: echo "" ;endif; endif; endif; ?>
	  </div>
	  <?php if(!empty($list['list'])): if(is_array($list['list'])): $i = 0; $__LIST__ = $list['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="listb">
  			<div class="bl">
			  <div class="pic"><img src="<?php echo ($vo['img']); ?>" border="0"/></div>
			</div>
			<div class="br link_gray6">
		  		<div class="t substring"><a href="<?php echo ($vo['url']); ?>"><?php echo ($vo['title']); ?></a></div>
	            <div class="time substring"><?php echo date('Y-m-d',$vo['addtime']);?></div>
  ...
  <div class="r">
  	...
  	<div class="newsso">
  	<form id="form" action="<?php echo url_rewrite('QS_newslist');?>">
		<input name="key" type="text" class="soinpu" value="<?php echo (urldecode(urldecode($_GET['key']))); ?>"/>
    ...
    <div class="cat">
		<ul class="link_gray6">
	  	<!--<li class="odd <if condition="$Think.get.id eq 0 || $Think.get.id eq ''" onclick="window.location='<?php echo url_rewrite('QS_newslist');?>'"><a href="<?php echo url_rewrite('QS_newslist');?>">全部</a></li>-->
	  	<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="<?php if($key%2 == 0): ?>odd<?php endif; ?> <?php if($_GET['id']== $vo['id']): ?>select<?php endif; ?>" onclick="window.location='<?php echo ($vo['url']); ?>'"><a href="<?php echo ($vo['url']); ?>"><?php echo ($vo['title_']); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
		</ul>
		<div class="clear"></div>
	</div>
			
	<?php $tag_news_list_class = new \Common\qscmstag\news_listTag(array('列表名'=>'hot_list','显示数目'=>'5','属性'=>'3','cache'=>'0','type'=>'run',));$hot_list = $tag_news_list_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"资讯中心 -  {categoryname} -{site_name}","keywords"=>"","description"=>"","header_title"=>""),$hot_list);?>
    ...
    <?php $tag_ad_class = new \Common\qscmstag\adTag(array('列表名'=>'ad','广告位名称'=>'QS_news_right','广告数量'=>'2','cache'=>'0','type'=>'run',));$ad = $tag_ad_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"资讯中心 -  {categoryname} -{site_name}","keywords"=>"","description"=>"","header_title"=>""),$ad);?>
    ...
    <?php $tag_news_list_class = new \Common\qscmstag\news_listTag(array('列表名'=>'recommend_list','显示数目'=>'5','属性'=>'4','cache'=>'0','type'=>'run',));$recommend_list = $tag_news_list_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"资讯中心 -  {categoryname} -{site_name}","keywords"=>"","description"=>"","header_title"=>""),$recommend_list);?>
...
<div class="footer_min" id="footer">
	...
	<?php $tag_explain_list_class = new \Common\qscmstag\explain_listTag(array('列表名'=>'list','cache'=>'0','type'=>'run',));$list = $tag_explain_list_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"资讯中心 -  {categoryname} -{site_name}","keywords"=>"","description"=>"","header_title"=>""),$list);?>
