...
class Qscms extends TagLib{
        ...
        if(C('DEFAULT_FILTER')) {
            ...
            foreach($filters as $filter){
                if(function_exists('array_walk_recursive')) {
                    array_walk_recursive($filter,$_POST);
                    array_walk_recursive($filter,$_GET);
                }else{
                    foreach ($args as $key => $val) {
                        $args[$key] = is_array($val)?array_map($filter,$val):$filter($val);
                    }
                }
            }
        }
    ...
    private static function config_seo() {
        ...
        return 'array("title"=>"'.$page['title'].'","keywords"=>"'.$page['keywords'].'","description"=>"'.$page['description'].'","header_title"=>"'.$page['header_title'].'")';
    }
    /**
     * 转换数据为HTML代码
     * @param array $data
